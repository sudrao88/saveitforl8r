    ...
    return (
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-1.5">
          {tags.map(tag => (
            <span key={tag} className="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1.5 animate-in fade-in zoom-in-50 duration-(--duration-fast)">
              #{tag}
          ...
          {SUGGESTED_TAGS.map(tag => {
            ...
            return (
              <button
                ...
                className={`${chip.base} ${chip.inactive} px-2.5 py-1 text-xs`}
              >
                + {tag}
              </button>
            );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {tags.map(tag => (
          <span key={tag} className="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 animate-in fade-in zoom-in-50 duration-(--duration-fast)">
            #{tag}
        ...
        {SUGGESTED_TAGS.map(tag => {
          ...
          return (
            <button
              key={tag}
              onClick={() => toggleTag(tag)}
              className={`${chip.base} ${chip.inactive} px-4 py-2`}
            >
              + {tag}
            </button>
          );
